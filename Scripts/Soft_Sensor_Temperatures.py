import joblib
import numpy as np
import pandas as pd

def run(csv_name):
    Append_Dataset = True

    print("---------------------------")
    print("Loading composition model...")

    model = joblib.load("Models/Temperature_predictor.pkl")
    scaler_X = joblib.load("Models/Temperature_predictorX.pkl")
    scaler_Y = joblib.load("Models/Temperature_predictorY.pkl")

    print("Loading dataset...")
    df = pd.read_csv(csv_name)
    n_trays = df["Number_of_Trays"][0]
    features = [
        "R_Noisy",
        "pf_Noisy",
        "steam_factor_Noisy",
        "X_Feed_Noisy",
        "X_Bottom_Noisy",
        "X_Top_Noisy",
        "Flow_Feed_Noisy",
        "Flow_Top_Noisy",
        "Flow_Bottom_Noisy"
    ]

    X_Input = df[features]

    print("Scaling inputs...")
    X = scaler_X.transform(X_Input)

    print("Predicting temperature profile...")
    pred = model.predict(X)
    pred = scaler_Y.inverse_transform(pred)

    if Append_Dataset:
        print("Appending predictions to dataset...")
        col_index = 0
        # Tf / Feed / Bottom / Top
        df["Tf_pred"] = pred[:, col_index]
        col_index += 1
        df["T_Bottom_pred"] = pred[:, col_index]
        col_index += 1
        df["T_Top_pred"] = pred[:, col_index]
        col_index += 1
        # Internal trays
        for tray in range(n_trays):
            if col_index >= pred.shape[1]:
                break
            df[f"T_{tray}_pred"] = pred[:, col_index]
            col_index += 1

    output_file = csv_name

    df.to_csv(output_file, index=False)

    print("\nDataset saved:", output_file)
    print("Script complete.")
